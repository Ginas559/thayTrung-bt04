...
import { getProductBySlugApi, getProductsApi } from '../util/api';
...
const ProductDetailPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const [productRes, productsRes] = await Promise.all([getProductBySlugApi(slug), getProductsApi()]);
    ...
    const similarProducts = useMemo(() => {
        if (!product) return [];
        return allProducts.filter((item) => item.slug !== product.slug && item.category === product.category).slice(0, 3);
    }, [allProducts, product]);

    const displayedImageIndex = Math.min(activeImageIndex, Math.max((product?.images?.length || 1) - 1, 0));
        ...
        return (
            <main className="bg-white px-4 pb-16 pt-6 sm:px-6 lg:px-8">
                <div className="mx-auto max-w-3xl rounded-[32px] border border-slate-200 bg-white p-8 shadow-sm">
                    ...
                    <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">Vui lòng đăng nhập để xem chi tiết sản phẩm</h1>
                    <p className="mt-3 text-base leading-8 text-slate-600">Dữ liệu sản phẩm đang lấy từ MongoDB thông qua API, nên bạn cần vào trang chủ sau khi đăng nhập thành viên.</p>
                    <div className="mt-6 flex gap-3">
                        <Link to="/login" className="inline-flex items-center gap-2 rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">
                            <span>Đăng nhập</span>
                        </Link>
                        <Link to="/" className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
    ...
    if (error || !product) {
        return (
            <main className="bg-white px-4 pb-16 pt-6 sm:px-6 lg:px-8">
                <div className="mx-auto max-w-3xl rounded-[32px] border border-slate-200 bg-white p-8 shadow-sm">
                    <p className="text-sm font-semibold uppercase tracking-[0.28em] text-slate-500">Chi tiết sản phẩm</p>
                    <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">Không tải được sản phẩm</h1>
                    <p className="mt-3 text-base leading-8 text-slate-600">{error || 'Sản phẩm không tồn tại.'}</p>
                    <div className="mt-6 flex gap-3">
                        <Link to="/" className="inline-flex items-center gap-2 rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">
                            <span>Về trang chủ</span>
                        </Link>
                    </div>
                </div>
                ...
                <section className="grid gap-10 lg:grid-cols-[1.08fr_0.92fr] lg:items-start">
                    <div className="space-y-6">
                                ...
                                {product.images.map((item, index) => (
                                    <SwiperSlide key={item}>
                                        <div className="flex aspect-[16/10] w-full items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.14),transparent_45%),linear-gradient(180deg,#111827_0%,#0f172a_100%)] px-4 py-4 sm:aspect-[16/9] sm:px-8 sm:py-8">
                                            <img src={item} alt={`${product.name} ${index + 1}`} loading={index === 0 ? 'eager' : 'lazy'} referrerPolicy="no-referrer" className="max-h-full w-auto max-w-full object-contain drop-shadow-[0_16px_40px_rgba(0,0,0,0.35)]" />
                                        </div>
                                    </SwiperSlide>
                                ))}

                                <button type="button" className="product-prev absolute left-4 top-1/2 z-10 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-slate-900 shadow-sm ring-1 ring-slate-200/70 transition hover:bg-white" aria-label="Ảnh trước">
                                    <LeftOutlined />
                                </button>

                                <button type="button" className="product-next absolute right-4 top-1/2 z-10 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-slate-900 shadow-sm ring-1 ring-slate-200/70 transition hover:bg-white" aria-label="Ảnh tiếp theo">
                                    <RightOutlined />
                                </button>
                            </Swiper>
                        </div>
                        ...
                        <div className="grid grid-cols-4 gap-3 sm:grid-cols-6">
                            {product.images.map((item, index) => (
                                <button
                                    ...
                                    className={`overflow-hidden rounded-[24px] ring-1 transition ${displayedImageIndex === index ? 'ring-slate-950 shadow-md' : 'ring-slate-200/70 hover:ring-slate-400'}`}
                                >
                                    <div className="bg-[#0f172a] p-2">
                                        <img src={item} alt={`${product.name} thumb ${index + 1}`} loading="lazy" referrerPolicy="no-referrer" className="h-16 w-full object-contain" />
                                    </div>
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-3">
                            <div className="rounded-[24px] bg-slate-50 px-5 py-5 text-sm leading-7 text-slate-600 ring-1 ring-slate-200/70">{product.highlight}</div>
                            <div className="rounded-[24px] bg-slate-50 px-5 py-5 text-sm leading-7 text-slate-600 ring-1 ring-slate-200/70">Layout: {product.category.split(' / ')[1]}</div>
                            <div className="rounded-[24px] bg-slate-50 px-5 py-5 text-sm leading-7 text-slate-600 ring-1 ring-slate-200/70">Số lượng bán được: {product.sold.toLocaleString('vi-VN')}</div>
                        </div>
                    </div>
                        ...
                        <div className="rounded-[36px] bg-slate-50 p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/70 sm:p-8">
                            ...
                            <div className="mt-6 rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-slate-200/60">
                                ...
                                <div className="mt-5 flex items-center gap-3">
                                    <button type="button" onClick={handleDecreaseQuantity} className="flex h-11 w-11 items-center justify-center rounded-full bg-slate-100 text-slate-700 transition hover:bg-slate-200">
                                        <MinusOutlined />
                                    </button>
                                    <div className="flex h-11 min-w-16 items-center justify-center rounded-full bg-slate-950 px-4 text-sm font-semibold text-white shadow-sm">{quantity}</div>
                                    <button type="button" onClick={handleIncreaseQuantity} className="flex h-11 w-11 items-center justify-center rounded-full bg-slate-100 text-slate-700 transition hover:bg-slate-200">
                                        <PlusOutlined />
                                    </button>
                            ...
                            <div className="mt-6 flex flex-wrap gap-3">
                                <button type="button" className="inline-flex items-center gap-2 rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">
                                    <ShoppingCartOutlined />
                                    <span>Thêm vào giỏ</span>
                                </button>
                                <button type="button" onClick={handleLogout} className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-900 transition hover:bg-slate-100">
                                    <UserOutlined />
                                    <span>Đăng xuất</span>
                                </button>
                            </div>
